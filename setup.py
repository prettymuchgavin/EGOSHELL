...
def _plain_setup() -> None:
    ...
    try:
        if sys.platform == "win32":
            sys.exit(subprocess.call([str(venv_python), __file__]))
        else:
...
class Installer:
    ...
    def _step_deps(self) -> None:
        ...
        pip = get_venv_pip() if VENV_DIR.exists() else shutil.which("pip3") or "pip"
    ...
    def _step_global(self) -> None:
        ...
        if sys.platform == "win32":
            egoshell_bin = bin_dir / "egoshell.cmd"
            script_content = textwrap.dedent(f"""\
                @echo off
                :: EgoShell global runner script for Windows
        ...
        try:
            ...
            if sys.platform == "win32":
                in_path = str(bin_dir).lower() in [p.lower() for p in path_env.split(";")]
            else:
            ...
            if not in_path:
                if sys.platform == "win32":
                    ...
                    add_path = Confirm.ask(
                        "  Would you like to automatically add this directory to your User PATH?",
                        default=True,
                        console=self.con,
                    )
                    if add_path:
                        try:
                            # PowerShell command to fetch, append, and set User PATH if not already present
                            ps_cmd = (
                            ...
                            self.con.print(
                                ...
                                f"    To add manually, run in PowerShell:\n"
                                f'    [cyan][Environment]::SetEnvironmentVariable("Path", [Environment]::GetEnvironmentVariable("Path", "User") + ";{bin_dir}", "User")[/cyan]'
                            )
                else:
                    self.con.print(
                        f"\n  [yellow]⚠[/yellow] Note: [bold]{bin_dir}[/bold] is not in your PATH.\n"
    ...
    def _step_service(self) -> None:
        self._section("⚙", "Background Service")

        self.con.print(
            "  [dim]You can install a background system service so the agent's\n"
            "  heartbeat loop runs automatically on startup in headless mode.[/dim]\n"
        )
        ...
        if not install_service:
            self.con.print("  [dim]Skipping background service installation.[/dim]")
            return
        ...
        if system == "Linux":
            service_path = Path.home() / ".config" / "systemd" / "user" / "egoshell.service"
            service_path.parent.mkdir(parents=True, exist_ok=True)
            content = textwrap.dedent(f"""\
                [Unit]
                Description=EgoShell Background Agent
                After=network.target

                [Service]
                ExecStart="{python_bin}" "{main_script}" --headless
                Restart=on-failure
                RestartSec=10
            ...
            try:
                ...
                subprocess.run(["systemctl", "--user", "enable", "egoshell.service"], capture_output=True)
                subprocess.run(["systemctl", "--user", "start", "egoshell.service"], capture_output=True)
                self.con.print("  [green]✓[/green] Created and started systemd user service (egoshell.service).")
            except Exception as e:
                self.con.print(f"  [red]✗[/red] Failed to set up systemd service: {e}")

        elif system == "Darwin": # macOS
            service_path = Path.home() / "Library" / "LaunchAgents" / "com.egoshell.agent.plist"
            service_path.parent.mkdir(parents=True, exist_ok=True)
            content = textwrap.dedent(f"""\
                <?xml version="1.0" encoding="UTF-8"?>
                <!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0/XMLPropertyLists-1.0.dtd">
                <plist version="1.0">
                <dict>
                    <key>Label</key>
                    <string>com.egoshell.agent</string>
            ...
            try:
                service_path.write_text(content, encoding="utf-8")
                subprocess.run(["launchctl", "load", "-w", str(service_path)], capture_output=True)
                self.con.print("  [green]✓[/green] Created and loaded launchd agent (com.egoshell.agent).")
            except Exception as e:
                self.con.print(f"  [red]✗[/red] Failed to set up launchd agent: {e}")

        elif system == "Windows":
            try:
                cmd = [
                    "schtasks", "/create", "/tn", "EgoShellAgent",
                    "/tr", f'cmd.exe /c start /min "" "{python_bin}" "{main_script}" --headless',
                    "/sc", "onlogon", "/f"
                ]
                subprocess.run(cmd, capture_output=True, check=True)
                self.con.print("  [green]✓[/green] Created Windows Scheduled Task (EgoShellAgent) running on logon.")
            except Exception as e:
                self.con.print(f"  [yellow]⚠[/yellow] Failed to set up Windows Scheduled Task (requires privileges): {e}")
                self.con.print("    Attempting fallback to Windows Startup folder...")
                try:
                    if "APPDATA" in os.environ:
                        startup_dir = Path(os.environ["APPDATA"]) / "Microsoft" / "Windows" / "Start Menu" / "Programs" / "Startup"
                        startup_dir.mkdir(parents=True, exist_ok=True)
                        bat_file = startup_dir / "egoshell_agent.bat"
                        content = textwrap.dedent(f"""\
                            @echo off
                            start /min "" "{python_bin}" "{main_script}" --headless
                        """)
                        bat_file.write_text(content, encoding="utf-8")
                        self.con.print("  [green]✓[/green] Created startup script in Windows Startup folder (no admin required).")
                    else:
                        raise OSError("APPDATA environment variable not found.")
                except Exception as ex:
                    self.con.print(f"  [red]✗[/red] Fallback to Startup folder failed: {ex}")
        else:
            self.con.print(f"  [yellow]⚠[/yellow] Background service setup not supported on {system}.")
